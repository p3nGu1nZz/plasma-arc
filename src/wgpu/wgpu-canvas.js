...
export class Canvas {
    static async createCanvas(state, canvas, ctx, config) {
        state.canvas = document.querySelector('canvas') || document.body.appendChild(document.createElement('canvas'));
        state.canvas.width = config.canvas.width;
        state.canvas.height = config.canvas.height;

        const glyphCanvas = Canvas._generateGlyphTextureAtlas(canvas, ctx, config);
    ...
    static _generateGlyphTextureAtlas(canvas, ctx, config) {
